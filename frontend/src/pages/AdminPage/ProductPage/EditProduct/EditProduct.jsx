    ...
    try {
      ...
      const payload = {
        ...
        variants: currentVariants.map(v => ({
          ...
          sku: v.sku || `${skuPrefix}-${v.size}-${v.color.name.toUpperCase().substring(0, 3)}`
        }))
      }
    ...
    <>
      ...
      >
        <DialogTitle sx={{ borderBottom: '1px solid #333', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="h6" fontWeight="bold">Chỉnh sửa sản phẩm</Typography>
        ...
        <DialogContent sx={{ p: 4 }}>
          {loading ? (
            ...
            <>
              ...
              <Box sx={{ mt: 5 }}>
                <Typography variant="subtitle2" sx={{ mb: 2, color: '#ccc' }}>Tồn kho theo biến thể</Typography>
