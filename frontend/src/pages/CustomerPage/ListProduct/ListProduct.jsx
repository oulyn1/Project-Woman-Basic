...
import {
  Box, Grid, Typography, Container, TextField, Button, 
  Select, MenuItem, FormControl, InputLabel, Divider, IconButton, InputBase, Paper
} from '@mui/material'
...
function ListProduct() {
  ...
  const processedProducts = useMemo(() => {
    ...
    return result
  }, [allProducts, categorySlug, categories, promotions, searchQuery, priceRange, sortBy])
          ...
          <Box sx={{ position: 'sticky', top: 120 }}>
            ...
            <Paper
              ...
              sx={{ 
                ...
                bgcolor: '#f2f2f2', borderRadius: 50, mb: 4
              }}
            >
              <InputBase
            ...
            {/* Price Filter */}
            <Typography variant="h6" fontWeight="bold" sx={{ mb: 2 }}>Giá tiền</Typography>
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 4 }}>
              {[
                ...
                <Button
                  key={btn.label}
                  variant={priceRange.min === btn.min && priceRange.max === btn.max ? "contained" : "outlined"}
                  onClick={() => handlePriceClick(btn.min, btn.max)}
                  sx={{ 
                    borderRadius: 1, textTransform: 'none', 
                    bgcolor: priceRange.min === btn.min && priceRange.max === btn.max ? '#ccc' : 'transparent',
                    color: 'black', borderColor: '#ccc',
            ...
            {/* Category Filter */}
            <Typography variant="h6" fontWeight="bold" sx={{ mb: 2 }}>Danh Mục Sản Phẩm</Typography>
        ...
        <Grid item xs={12} md={9}>
          ...
          {processedProducts.length === 0 ? (
            <Box sx={{ textAlign: 'center', py: 10 }}>
              <Typography variant="h5" color="text.secondary">Không tìm thấy sản phẩm phù hợp.</Typography>
            </Box>
          ) : (
